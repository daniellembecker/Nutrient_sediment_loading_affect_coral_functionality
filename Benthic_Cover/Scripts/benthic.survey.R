...
benthic.dat <- read_csv("Benthic_Cover/Data/benthic.survey.csv")
view(benthic.dat)
...
nut.dat <- read.csv("Effect_Sizes/Data/covariate.data.csv")
...
ggplot(tot.dat, aes(x=N.resid, y=percent.cover)) + #set aesthetics for my x and y usuing means for x and y vars
  ...
  geom_text(data = tot.dat, aes(label = site.letter), vjust = 0.9, hjust = 2.0) + #add labels for each site 
...
ggsave(filename = "Benthic_Cover/Output/percent.cover.p.acuta.pdf", device = "pdf", width = 7, height = 5)
...
ggplot(tot.dat, aes(x=trap.resid, y=percent.cover)) + #set aesthetics for my x and y usuing means for x and y vars
  geom_point() +
  geom_text(data = tot.dat, aes(label = site.letter), vjust = 0.5, hjust = 2.0) + #add labels for each site 
  theme_bw()+ #overall theme for plot
...
ggsave(filename = "Benthic_Cover/Output/trap.percent.cover.pdf", device = "f", width = 7, height = 5)
...
write.csv(tot.dat, file = "Benthic_Cover/Data/percent.cover.csv")
