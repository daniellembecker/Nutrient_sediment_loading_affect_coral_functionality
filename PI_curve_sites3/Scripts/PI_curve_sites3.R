...
# get the file path
setwd("Summer_2019/PI_curve_sites3/")
path.p<-"../PI_curve_sites3/Data/PI_curve_resp" #the location of all your respirometry files 
...
for(i in 1:length(file.names.full)) { # for every file in list calculate O2 uptake or release rate and add the data to the Photo.R dataframe
  ...
  Regs  <-  rankLocReg(xall=Photo.Data1$sec, yall=Photo.Data1$Value, alpha=0.67, method="pc", verbose=TRUE)  
...
curve.nlslrc= nls(Pc ~ (1/(2*theta))*(AQY*PAR+Am-sqrt((AQY*PAR+Am)^2-4*AQY*theta*Am*PAR))-Rd,start=list(Am=(max(Pc)-min(Pc)),AQY=0.001,Rd=-min(Pc),theta=0.97)) 
...
text(x = 440, y = 1.0, label = "Ik", srt = 0)

dev.copy(png,'Output/Moorea_Sum19_PI_Sites3.png', width = 5, height = 4, units = "in", res = 1200)
dev.off()
